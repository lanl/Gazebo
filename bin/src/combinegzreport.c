...
int main( int argc, char *argv[] ) 
{
 ...
 int attransition = 0 ;
 int JobsDescribed = 0, MaxJobid = -1 ;
...
int countdifferror = 0, countsameerror = 0 ;
 ...
 if ( argc < 4 ) {
  printf( "Usage: ./combinegzreport gazebo.results.date.txt CommonErrReport.txt OutputGazeboPatchedResults.txt\n" ) ;
  exit( 0 ) ;
 }
 ...
 fgets( cur_line, 2280, fin2 ) ;
 for ( linenum = 0 ; fgets( cur_line, 2280, fin2 ) != NULL ; linenum++ ) {
 } 
